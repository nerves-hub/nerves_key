defmodule ATECC508A.Request do
  ...
  """
  @spec read_zone(zone(), slot(), block(), offset(), access_size()) :: transaction()
  def read_zone(zone, slot, block, offset, length) do
    addr = get_addr(zone, slot, block, offset)

    {<<7, @atecc508a_op_read, length_flag(length)::1, 0::5, zone_index(zone)::2, addr::binary>>,
     5000, length}
  end
  ...
  def write_zone(zone, slot, block, offset, data) do
    addr = get_addr(zone, slot, block, offset)

    len = byte_size(data)

    {<<len + 7, @atecc508a_op_write, length_flag(len)::1, 0::5, zone_index(zone)::2, addr::binary,
       data::binary>>, 5000, 1}
  end
  ...
  """
  @spec genkey(any(), any()) :: transaction()
  def genkey(mode, key_id) do
    {<<7, @atecc508a_op_genkey, mode, key_id>>, 32, 1}
  end
  ...
  def lock_zone(zone, zone_crc) do
    ...
    {<<7, @atecc508a_op_lock, mode, zone_crc::binary>>, 32, 1}
  end
